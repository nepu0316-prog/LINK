...
export function VideosClient({ initialVideos }: Props) {
  ...
  function openEdit(v: Video) {
    setForm({ title: v.title, description: v.description || '', thumbnail_url: v.thumbnail_url, video_url: v.video_url, platform: v.platform, is_pinned: v.is_pinned, is_published: v.is_published, sort_order: v.sort_order, category: (v as any).category || '' })
    setEditingId(v.id)
    setModalOpen(true)
  }
              ...
              {videos.map((v, i) => (
                <motion.div key={v.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ delay: i * 0.03 }} className="card-soft p-4 flex items-center gap-4">
                  <div className="w-20 h-12 rounded-xl overflow-hidden flex-shrink-0 bg-cream-100 dark:bg-warm-700">
                    {v.thumbnail_url ? <Image src={v.thumbnail_url} alt={v.title} width={80} height={48} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-xl">🎬</div>}
                  </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? '編輯影片' : '新增影片'} size="lg">
        <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
          <div>
            ...
            <input value={form.video_url} onChange={e => setForm(f => ({ ...f, video_url: e.target.value }))} onBlur={e => fetchIGoembed(e.target.value)} placeholder="Instagram Reels 網址" type="url" className="input-field" />
          </div>
          <div>
          ...
          <div>
            <label className="block text-xs font-medium text-warm-600 dark:text-warm-400 mb-1.5">自訂縮圖（可選）</label>
